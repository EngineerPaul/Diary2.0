...
_SECRETS_FILE = "tgserver_secrets"
...
def parse_secrets_file(file_path: str) -> dict:
    ...
    try:
        ...
        msg = f"Warning: Could not parse secrets file {file_path}: {e}"
        print(msg)
...
def get_secret(secret_name: str, default: Any = None) -> Any:
    ...
    secret_path = f"/run/secrets/{_SECRETS_FILE}"
    ...
    # Если Docker secrets не найдены,
    # используем переменные окружения (development)
